...
type State =
  ...
  | { readonly status: "ready"; readonly value: GetTrialBalanceResponse };
...
export function TrialBalanceScreen({ api, now, workspace }: TrialBalanceScreenProps) {
  ...
  useEffect(() => {
    ...
    const request = create(GetTrialBalanceRequestSchema, {
      ...
      asOfDate: create(CivilDateSchema, {
        year,
        month,
        day,
      }),
    ...
    void api
      .getTrialBalance(codec.encodeRequest(request))
    ...
    <div className="mx-auto grid w-full max-w-[920px] gap-5">
      <div>
        ...
        <p className="mt-1 text-[11px] leading-5 text-muted-foreground">
          Debit and credit balances as at {instant.toLocaleDateString("en-AU")}.
        </p>
      </div>
          ...
          <table className="w-full border-collapse text-left">
            <thead className="border-b border-border bg-background/60 text-[9px] uppercase text-muted-foreground">
              <tr>
                ...
                <th className="px-3 py-2 text-right">Debit (AUD)</th>
                <th className="px-3 py-2 text-right">Credit (AUD)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border text-[10px]">
              {state.value.lines.map((line) => (
                <tr key={line.account?.id ?? line.code}>
                  <td className="px-3 py-2.5">
                    <span className="mr-3 font-mono text-muted-foreground">{line.code}</span>
                    <span className="font-medium">{line.name}</span>
                  </td>
                  <td className="px-3 py-2.5 text-right font-mono">
                    {moneyLabel(line.debits?.minorUnits ?? 0n)}
                  </td>
                  <td className="px-3 py-2.5 text-right font-mono">
                    {moneyLabel(line.credits?.minorUnits ?? 0n)}
                  </td>
                </tr>
              ))}
            </tbody>
            <tfoot className="border-t border-border text-[10px] font-semibold text-forest">
              <tr>
                <td className="px-3 py-2.5">Total</td>
                <td className="px-3 py-2.5 text-right font-mono">
                  {moneyLabel(state.value.totalDebits?.minorUnits ?? 0n)}
                </td>
                <td className="px-3 py-2.5 text-right font-mono">
                  {moneyLabel(state.value.totalCredits?.minorUnits ?? 0n)}
                </td>
              </tr>
            </tfoot>
          </table>
          ...
          <div className="grid min-h-64 place-items-center text-center">
            <div>
              ...
              <p className="mt-1 text-[10px] text-muted-foreground">
                Post a journal to establish a financial revision.
              </p>
            </div>
          </div>
        )}
