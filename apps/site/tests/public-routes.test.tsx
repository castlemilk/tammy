...
describe('Tammy public routes', () => {
  ...
  it('renders the canonical app/site/support privacy boundaries', () => {
    ...
    expect(
      screen.getByText(/does not transmit your accounting records/i),
    ).toBeTruthy();
    expect(
      screen.getByText(
        /hosting infrastructure may process request and security logs/i,
      ),
    ).toBeTruthy();
    expect(
      screen.getByText(
        /messages are processed by the relevant email providers/i,
      ),
    ).toBeTruthy();
  });
  ...
  it('renders only the canonical support and deletion inventory', () => {
    ...
    expect(
      screen
        .getByRole('link', { name: /ben\.ebsworth@gmail\.com/i })
        .getAttribute('href'),
    ...
    expect(
      screen.getByText(/app deletion alone does not remove/i),
    ).toBeTruthy();
  });
});
