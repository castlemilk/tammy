...
| Slow or oversized local requests | Configure a 2-second header timeout, 5-second whole-request read timeout, 5-second response write timeout, 30-second idle timeout, and 16 KiB maximum HTTP header size. Limit each decoded Connect request message to 1 MiB and wrap the complete mux with a 1,048,581-byte total HTTP body limit: one 1 MiB message plus the 5-byte Connect envelope. The client request timeout is 5 seconds. | `services/core/internal/transport/server_integration_test.go` exercises partial TLS handshakes, incomplete headers and bodies, completed oversized bodies with missing and valid capabilities, the exact HTTP/Connect resource-exhausted response, a just-under-limit valid call, and graceful shutdown; `apps/desktop/src/main/core-client.test.ts` |
