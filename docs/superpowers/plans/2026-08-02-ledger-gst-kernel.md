...
- [ ] Update `coverage.yaml` in the same change with each fully-qualified RPC and lifecycle transition staged as `declared_future`: keep executed `cases` empty, map non-empty `futureCases` to scenario `E2E-00`, `E2E-01`, or `E2E-14`, and declare the planned named preload, routes, all four role outcomes, principal failures, list states, replay/conflict cases, and projections. Do not expose or add placeholder preload methods in this task.
...
- [ ] As each real named preload/UI path becomes available, move its scenario IDs from `futureCases` to executed `cases` and promote its RPC and lifecycle transitions from `declared_future` to `production`. Before committing Task 12, promote every Slice 1 entry and run `rtk pnpm contracts:production`; any `E2E_COVERAGE_FUTURE_PROMOTION_REQUIRED` result blocks completion.
...
- [ ] Run `rtk pnpm exec node --test scripts/generate-pre-restore-age-fixture.test.mjs && rtk go test ./tools/restore-fixture/...`, then `rtk pnpm contracts && rtk pnpm contracts:production && rtk pnpm lint && rtk pnpm typecheck && rtk pnpm test`; future-stage coverage blocks evidence and packaged acceptance.
...
- [ ] All Slice 1 RPCs, transitions, roles, replay/conflict paths, typed failures, and list states are production entries in `test/e2e/coverage.yaml`; both `rtk pnpm contracts` and `rtk pnpm contracts:production` pass.
