...
- The semantic idempotency hash covers a stable semantic-hash algorithm version, the fully-qualified message name, and the RFC 8785 encoding of the normalized Protobuf JSON mapping after authentication metadata and idempotency key are removed. It deliberately excludes the release-wide descriptor fingerprint, so an unrelated compatible schema addition cannot invalidate a workspace-lifetime replay.
- Each idempotency row stores its semantic-hash algorithm version. Replay recomputes with that stored version. A later algorithm version applies only to previously unseen operation keys unless a reviewed migration proves byte-for-byte equivalence for every retained fixture.
...
- A newly added presence-aware field that is absent is omitted and therefore preserves an old request hash. If the caller explicitly sets that new field, normalized JSON includes it and reuse of the old key correctly returns `IDEMPOTENCY_CONFLICT`.
...
The ordinary `pnpm contracts` gate accepts complete staged declarations so descriptor work and production UI work can land in their owning tasks. `pnpm contracts:production` validates the shipping surface: production RPCs and transitions require executed cases, while declared-future entries remain permitted only when they have no executed cases and no renderer preload exposure. Exposing a future RPC still fails with `E2E_COVERAGE_FUTURE_PROMOTION_REQUIRED`. Task 1 declares Slice 1 coverage, Task 12 adds the real named preload/UI paths and promotes every implemented Slice 1 entry, and Task 13 records runtime coverage and retained evidence from production entries only.
