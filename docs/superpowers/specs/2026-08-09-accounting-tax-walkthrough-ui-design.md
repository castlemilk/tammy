...
- Australian organisation defaults, AUD presentation, accrual GST, and a standard small-business chart template.
...
Blank setup collects business name, ABN when known, financial-year end, GST registration date, GST basis, reporting frequency, and administrator credentials. Setup installs the Australian chart template and opens the Overview. Optional sample data is an explicit unchecked choice and can never be silently added to a real workspace.
...
The review screen follows the approved split view:
...
- bottom: one **Approve and post** action.

The reviewer can reject a candidate, correct it, or cancel. Arithmetic, GST, required-field, duplicate, and posting-date errors appear next to the relevant fields. No document is auto-posted.

### 5.3 Banking

Banking shows financial accounts, imported statements, and staged lines. Its primary action is **Import statement**. The walkthrough supports CSV and extracted PDF statements through the same staging model.

The user confirms the account, date/amount mapping, sign convention, statement range, opening/closing balances, and duplicates before committing the import. A staged payment can be matched to the posted supplier document. Completion records the match and shows the resulting reconciled/unreconciled status without creating duplicate journals.
...
The primary action is **Review BAS draft**. Review validates missing evidence and provenance, then retains a local workpaper/version. This milestone does not expose a Lodge or Submit control.
...
The existing Workspace, Identity, Organisation, Accounting, and Audit services remain authoritative. Focused contracts are added for:

- `DocumentService`: import evidence, get/list documents, get/cancel/retry extraction jobs, provide an in-memory PDF password, save review, and approve a reviewed target;
- `BankingService`: create/list financial accounts, stage/import statements, list staged lines, commit an import, create/confirm matches, and query reconciliation status;
- `ReportingService`: overview attention summary, GST detail, BAS draft creation/version query, and local workpaper export.
...
```text
RETAINED → QUEUED → EXTRACTING → NEEDS_REVIEW → REVIEWED → POSTED
...
Only `REVIEWED → POSTED` changes accounting. Approval runs in one encrypted SQLite write transaction and either commits all of the following or none:

- immutable review result;
- target supplier expense/payable or paid-expense record;
- balanced journal and lines;
- retained GST facts;
- document-to-journal provenance;
- audit event and idempotency result.

If posting fails, the document remains reviewed and editable/retryable; no partial ledger, GST, match, or audit success is visible. Replaying the same semantic approval with the same operation key returns the same result. A changed request conflicts.
...
5. review the source beside fields, correct at least one candidate, and approve;
6. verify the balanced journal, ledger movement, trial balance, GST purchase credit, BAS draft, and linked audit event;
7. import a bank CSV and extracted PDF statement, review signs/duplicates, match the supplier payment, and inspect reconciliation status;
...
2. Complete the existing accounting kernel composition and expose chart, manual journals, ledger, and trial balance through named preload methods.
3. Add document/evidence Protobuf contracts, encrypted persistence, restart-safe job lifecycle, and list/detail UI.
4. Package the local PDF classification/text helper and lightweight OCR provider, then connect candidate extraction to the review UI.
5. Implement atomic reviewed-document posting with GST and end-to-end source provenance.
6. Add banking statement staging/matching and the walkthrough reconciliation projection.
7. Add GST detail, draft BAS workpaper, audit trace links, Settings/diagnostics, exports, and recovery states.
8. Harden all paths with hostile-document, rollback, restart, offline, accessibility, and packaged walkthrough gates.
...
- one approved supplier document produces a balanced, traceable journal and GST fact atomically;
- bank statement import and supplier-payment matching work through the UI;
