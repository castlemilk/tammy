...
The existing App ID and App Store Connect record belong to an individual membership until Apple verifies an organization conversion, so the design includes an explicit migration branch. The preferred path is Apple's supported update of the existing individual membership to Gamma Systems Pty Ltd; transfer or recreation is required only if Apple directs it. The approved organization membership may retain or replace the historical Team ID, so eligibility is determined from Apple's verified seller and membership state rather than the Team ID string alone. After approval, Tammy must bind every identifier, certificate, and freshly issued profile to the resulting organization-controlled team. Release checks then verify `ElectronTeamID`, application identifier, keychain/application groups, helper identifiers, embedded profile, App Store Connect ID, and seller metadata together. No candidate qualifies for `PRE_UPLOAD_READY` while Apple still identifies the membership and seller as the individual unless the written-Apple-exception branch is recorded.
