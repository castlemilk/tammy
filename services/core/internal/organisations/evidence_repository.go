...
// GetCurrentSummary returns only state and expiry from the latest integrity-verified retained
// evidence pair. Raw evidence, semantic hashes, and protected-storage identifiers remain internal.
func (repository *EvidenceRepository) GetCurrentSummary(ctx context.Context, organisationID string) (*tammyv1.OrganisationVerificationSummary, error) {
	...
	err := repository.tx.QueryRowContext(ctx, `
		...
		ORDER BY recorded_at DESC, id DESC LIMIT 1`, organisationID).Scan(&verificationID)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	}
	if err != nil {
		return nil, fmt.Errorf("organisations: load current verification summary: %w", err)
	}
	if !ids.IsCanonicalV7(verificationID) {
...
func currentVerificationSummary(record VerificationRecord, organisationID string) (*tammyv1.OrganisationVerificationSummary, error) {
	...
	if record.Verification == nil || record.Verification.OrganisationId != organisationID ||
		...
		!record.Verification.ExpiresAt.IsValid() {
		return nil, ErrEvidenceTampered
	}
	return &tammyv1.OrganisationVerificationSummary{
		State: record.Verification.State, ExpiresAt: proto.Clone(record.Verification.ExpiresAt).(*timestamppb.Timestamp),
	}, nil
}
