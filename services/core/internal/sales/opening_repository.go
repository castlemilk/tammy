...
var ErrOpeningRepository = errors.New("sales: opening repository failure")
...
func (repository *OpeningRepository) RecordOpeningReceivable(ctx context.Context, conversionID string,
	...
	if repository == nil || repository.executor == nil || ctx == nil || !ids.IsCanonicalV7(conversionID) || input == nil ||
		...
		(input.Kind != tammyv1.OpeningBalanceKind_OPENING_BALANCE_KIND_CUSTOMER_OPEN_ITEM &&
	...
	if _, err := repository.executor.ExecContext(ctx, `INSERT INTO sales_opening_receivables(id, conversion_id, retained_input_proto, created_at) VALUES(?,?,?,?)`,
